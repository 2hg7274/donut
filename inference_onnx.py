...
def inference_onnx(path, donut_model_path, image_path, prompt):
    ...
    if 'onnx_quantized' in path:
        encoder_sess = ort.InferenceSession(os.path.join(path, 'quantized_encoder.onnx'))
        decoder_sess1 = ort.InferenceSession(os.path.join(path, 'quantized_decoder.onnx'))
        decoder_sess2 = ort.InferenceSession(os.path.join(path, 'quantized_decoder_with_past.onnx'))
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--onnx_save_path', type=str, default='result/onnx')
    parser.add_argument('--donut_model_path', type=str, required=True)
    parser.add_argument('--image_path', type=str, required=True)
    parser.add_argument('--prompt', type=str, required=True)
    args, left_argv = parser.parse_known_args()

    res, predict = inference_onnx(path=args.onnx_save_path, donut_model_path=args.donut_model_path ,image_path=args.image_path, prompt=args.prompt)
    print(predict)